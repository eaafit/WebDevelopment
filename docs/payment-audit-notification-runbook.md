...
Runbook можно использовать перед демонстрацией лабораторной работы,
после изменения платежной логики и при разборе локальных ошибок.
...
- Сохранить понятный сценарий защиты.
...
| Нотариус видит только свое | защита данных | чужие payment-only events скрыты |
...
- Реализована связка платежей, аудита и уведомлений.
- Ошибки Robokassa теперь фиксируются как отдельные audit events.
- Уведомления работают в best-effort режиме и не ломают оплату.
- Админская таблица платежей устойчивее к длинным идентификаторам.
- CSV-экспорт сообщает оператору о проблемах скачивания.
- Список заявок имеет fallback при проблемах связанных записей.
- Нотариальный scope не раскрывает чужие события.
- Проверки покрыты тестами admin и assessment.
...
## Данные для ручной демонстрации

| Данные | Пример | Где использовать |
| --- | --- | --- |
| paymentId | UUID или numeric id | таблица платежей |
| assessmentId | UUID заявки | переход к заявке |
| transactionId | provider id | поиск и CSV |
| userId | UUID пользователя | fallback имени |
| amount | `2500.00` | Robokassa OutSum |
| status | `pending` или `completed` | фильтр статуса |
| eventType | `payment.created` | мониторинг |
| targetType | `Payment` | мониторинг |
| category | `Payment` | notification center |
| readAt | timestamp или пусто | read state |

## Контрольные вопросы для самопроверки

| Вопрос | Правильный ответ |
| --- | --- |
| Что произойдет, если уведомление не создастся? | платежный сценарий продолжится |
| Где увидеть ошибку Robokassa? | в audit monitoring |
| Где увидеть успешный redirect? | в payment audit event |
| Что делает summary fallback? | возвращает базовые поля заявки |
| Почему ID-колонка компактная? | чтобы таблица помещалась на экран |
| Почему CSV не должен писать ложный success? | оператор должен видеть реальную ошибку |
| Что проверяет notary scope? | отсутствие чужих событий |
| Что хранит notification category? | область события |
| Что означает readAt? | уведомление прочитано |
| Что проверяет callback signature? | подлинность ответа Robokassa |
| Что проверяет OutSum? | совпадение суммы платежа |
| Что делает duplicate callback? | не создает второй бизнес-эффект |
| Что делать при пустом CSV? | показать ошибку выбора |
| Что делать при broken relation? | включить fallback чтение |
| Что должно быть в audit context? | безопасные диагностические поля |
| Что не должно быть в audit context? | секреты, токены, приватные ключи |
| Что делает route `/admin/orders`? | показывает список заявок |
| Что делает query `assessmentId`? | помогает открыть связанную заявку |
| Что проверяет production build? | компиляцию Angular в production |
| Что проверяет affected test? | регрессию по измененным проектам |

## Быстрый порядок демонстрации

| Порядок | Действие | Комментарий |
| --- | --- | --- |
| 1 | Запустить API и Web | использовать локальный bat или `pnpm.cmd nx serve` |
| 2 | Открыть админку | проверить отсутствие raw fallback-текстов |
| 3 | Открыть платежи | показать таблицу |
| 4 | Изменить ширину ID | показать UX-доработку |
| 5 | Сделать CSV-export | показать файл или ошибку |
| 6 | Перейти к заявкам | показать связку payment -> assessment |
| 7 | Открыть monitoring | показать audit trail |
| 8 | Открыть notifications | показать category/read state |
| 9 | Рассказать про Robokassa failure | показать event и best-effort |
| 10 | Рассказать про тесты | показать команды проверки |

## Итог

Runbook фиксирует ожидаемое поведение платежей, аудита и уведомлений.
Он помогает быстро проверить лабораторную работу перед демонстрацией.
