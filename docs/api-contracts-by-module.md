...
Продуктовый audit trail: события действий пользователей и доменных операций, backend-пагинация, фильтры `event_type`, `actor_query`, `actor_user_id`, `target_id`, `assessment_id`, `date_range`, CSV-экспорт. `/admin/monitoring` видит все события, `/notary/monitoring` получает только события по заявкам текущего нотариуса через `assessment_id`/notary scope.
