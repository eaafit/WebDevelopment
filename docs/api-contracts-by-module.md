...
| API-сервис       | RPC-методы                         |
| ---------------- | ---------------------------------- |
| **AuditService** | ListAuditEvents, ExportAuditEvents |

Продуктовый audit trail: события действий пользователей и доменных операций, backend-пагинация, фильтры `event_type`, `actor_query`, `actor_user_id`, `target_id`, `date_range`, CSV-экспорт. Audit-событие привязывается к объекту через универсальные `entity_name` + `entity_id`; для событий заявки используется `entity_name = Assessment`, `entity_id = id заявки`, поэтому UI-фильтр «ID заявки» отправляет `target_id`. `/admin/monitoring` видит все события, `/notary/monitoring` получает только события по заявкам текущего нотариуса через список его assessments и `entity_id`.

События безопасности и отдельный просмотр логов по пользователю/заказу относятся к issue 23. Техническое JSON-логирование, Pino, request-id, Loki/Grafana относятся к issue 32/33 и не входят в этот контракт.
