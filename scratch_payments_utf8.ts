...
export class Payments implements OnInit, OnDestroy {
  ...
  readonly headerColumns: { key: FilterColumn; label: string }[] = [
    { key: 'id', label: 'ID' },
    { key: 'paymentDate', label: '╨Ф╨░╤В╨░' },
    { key: 'payer', label: '╨Я╨╗╨░╤В╨╡╨╗╤М╤Й╨╕╨║' },
    { key: 'type', label: '╨в╨╕╨┐' },
    { key: 'amount', label: '╨б╤Г╨╝╨╝╨░' },
    { key: 'fee', label: '╨Ъ╨╛╨╝╨╕╤Б╤Б╨╕╤П' },
    { key: 'paymentMethod', label: '╨Ь╨╡╤В╨╛╨┤' },
    { key: 'transactionId', label: 'ID ╤В╤А╨░╨╜╨╖╨░╨║╤Ж╨╕╨╕' },
    { key: 'attachment', label: '╨з╨╡╨║' },
    { key: 'application', label: '╨Ч╨░╤П╨▓╨║╨░' },
    { key: 'status', label: '╨б╤В╨░╤В╤Г╤Б' },
    { key: 'actions', label: '╨Ф╨╡╨╣╤Б╤В╨▓╨╕╤П' },
  ];
  ...
  ngOnInit(): void {
    ...
    this.dataSub = this.api.payments$.subscribe({
      ...
      error: (err) => {
        ...
        this.payments = [];
        this.loadError = '╨Э╨╡ ╤Г╨┤╨░╨╗╨╛╤Б╤М ╨╖╨░╨│╤А╤Г╨╖╨╕╤В╤М ╨┤╨░╨╜╨╜╤Л╨╡ ╨┐╨╗╨░╤В╨╡╨╢╨╡╨╣ ╤Б ╤Б╨╡╤А╨▓╨╡╤А╨░';
        this.loading = false;
      },
    });
  }
  ...
  async onDeleteConfirmed(): Promise<void> {
    ...
    try {
      ...
      this.loadError = err instanceof Error ? err.message : '╨Ю╤И╨╕╨▒╨║╨░ ╨┐╤А╨╕ ╤Г╨┤╨░╨╗╨╡╨╜╨╕╨╕ ╨┐╨╗╨░╤В╨╡╨╢╨░';
    }
  }
  ...
  getMethodLabel(method?: PaymentMethod): string {
    return method ? PAYMENT_METHOD_LABELS[method] : 'тАФ';
  }
  ...
  getCellValue(payment: Payment, column: FilterColumn): string {
    switch (column) {
      ...
      case 'transactionId':
        return payment.transactionId || 'тАФ';
      case 'attachment':
        return payment.attachmentFileName || 'тАФ';
      case 'application':
        return payment.assessmentId || payment.subscriptionId || 'тАФ';
      case 'status':
        return this.getStatusLabel(payment.status);
      case 'actions':
        return payment.assessmentId
          ? '╨Я╤А╨╛╤Б╨╝╨╛╤В╤А, ╨а╨╡╨┤╨░╨║╤В╨╕╤А╨╛╨▓╨░╤В╤М, ╨г╨┤╨░╨╗╨╕╤В╤М, ╨Ч╨░╤П╨▓╨║╨░'
          : '╨Я╤А╨╛╤Б╨╝╨╛╤В╤А, ╨а╨╡╨┤╨░╨║╤В╨╕╤А╨╛╨▓╨░╤В╤М, ╨г╨┤╨░╨╗╨╕╤В╤М';
      default:
        return '';
    }
  }
  ...
  private buildCsvContent(payments: Payment[]): string {
    const rows = payments.map((payment) => [
      ...
      payment.transactionId || 'тАФ',
      payment.attachmentFileName || 'тАФ',
      payment.assessmentId || payment.subscriptionId || 'тАФ',
